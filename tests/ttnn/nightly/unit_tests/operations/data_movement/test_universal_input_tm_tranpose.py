...
def test_transpose_specless_sharded_output_grid_shrinks_block_col_major(device):
    """BLOCK_SHARDED no-spec output with COL_MAJOR input: physical roles swap → phys_x=n_h, phys_y=n_w.
    Distinguishes correct COL_MAJOR handling from a ROW_MAJOR-fallback regression on any grid; on non-square
    grids (BH) it also catches the bad-clamp bug where phys_x was clamped by grid.y instead of grid.x.
    input=(1,1,96,64) COL_MAJOR BLOCK → transpose(2,3) → out=(1,1,64,96); n_h=2, n_w=3 → CoreRange((0,0),(1,2))."""
    shape = (1, 1, 96, 64)
    compute_grid = device.compute_with_storage_grid_size()
