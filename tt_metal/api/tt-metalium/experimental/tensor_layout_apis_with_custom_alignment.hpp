...
/**
 * Same as tensor_layout_with_custom_alignment, for restoring a TensorLayout from a
 * serialized representation that carries an Alignment of its own.
 *
 * Deserialization has to be able to reproduce whatever Alignment was serialized,
 * including one that no longer has a way to be constructed through the graduated API.
 */
